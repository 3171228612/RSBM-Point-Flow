...
try:
    ...
    import warnings
    warnings.warn(
        "flash-attn not installed. Using PyTorch SDPA fallback. "
        "Training will be slower but functionally correct."
    )
...
class DiTLayer(nn.Module):
    ...
    def _part_attention(self, x: torch.Tensor, cu_seqlens: torch.Tensor, max_seqlen: int) -> torch.Tensor:
        ...
        if HAS_FLASH_ATTN:
            ...
            out = self._varlen_attn_fallback(
                qkv.to(self.attn_dtype), cu_seqlens
            ).to(x.dtype)
    ...
    def _global_attention(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if HAS_FLASH_ATTN:
            ...
            q = q.transpose(1, 2).to(self.attn_dtype)                   # (B, H, N, D)
            k = k.transpose(1, 2).to(self.attn_dtype)
            v = v.transpose(1, 2).to(self.attn_dtype)
            out = F.scaled_dot_product_attention(q, k, v)               # (B, H, N, D)
            out = out.transpose(1, 2).to(x.dtype)                       # (B, N, H, D)

        out = out.view(B, N, self.dim)                                   # (B, N, embed_dim)
        return self.global_out_proj(out)

    def _varlen_attn_fallback(self, qkv: torch.Tensor, cu_seqlens: torch.Tensor) -> torch.Tensor:
        """Fallback for flash_attn_varlen_qkvpacked_func using PyTorch SDPA.
        
        Args:
            qkv: (total_tokens, 3, H, D)
            cu_seqlens: (num_segments + 1,)
        ...
        for i in range(num_segments):
            ...
            if start >= end:
                continue
            seg_qkv = qkv[start:end]                                    # (L, 3, H, D)
            q, k, v = seg_qkv.unbind(dim=1)                             # each (L, H, D)
            q = q.permute(1, 0, 2).unsqueeze(0)                         # (1, H, L, D)
            k = k.permute(1, 0, 2).unsqueeze(0)
            v = v.permute(1, 0, 2).unsqueeze(0)
            attn_out = F.scaled_dot_product_attention(q, k, v)           # (1, H, L, D)
            out[start:end] = attn_out.squeeze(0).permute(1, 0, 2)       # (L, H, D)

        return out

    # @torch.compile  # disabled: PyTorch 2.8 has a torch._inductor bug with CuteDSLBenchmarkRequest
    def forward(
        self,
